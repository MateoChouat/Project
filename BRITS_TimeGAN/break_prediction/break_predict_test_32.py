# -*- coding: utf-8 -*-
"""
Version enrichie avec DBSCAN, K-Means, Optuna et Bootstrap
"""
import pandas as pd
...
def apply_kmeans(X_train_scaled, k_values):
    results = Parallel(n_jobs=-1)(delayed(kmeans_clustering)(k, X_train_scaled) for k in k_values)
    best_k = k_values[np.argmax([result[1] for result in results])]
...
def prediction(CANA_PATH, DEFA_PATH):
    ...
    # Enlever bruit (-1) si besoin
    #X_train = X_train[X_train['cluster'] != -1]
    #y_train = y_train[y_train['cluster'] != -1]
    
    # Vérification des tailles
    print("Taille de X_train après DBSCAN :", X_train.shape)
    
    # 5. Application de K-Means
    k_values = range(2, 11)  # Tester pour 2 à 10 clusters
    best_k, kmeans_results = apply_kmeans(X_train_scaled, k_values)
    
    print(f"Meilleur nombre de clusters K-Means : {best_k}")
    
    # Vérification que la taille des labels K-Means correspond
    kmeans_labels = kmeans_results[best_k - 2][2]  # Obtenir les labels du meilleur k
    print("Taille des labels K-Means :", len(kmeans_labels))
    
    # Réinitialisation des indices pour aligner les données avec les labels K-Means
    X_train = X_train.reset_index(drop=True)
    
    # Filtrer les labels K-Means pour correspondre à la taille de X_train après DBSCAN
    kmeans_labels_filtered = kmeans_labels[:len(X_train)]
    
    # Assigner les labels K-Means
    X_train['kmeans_cluster'] = kmeans_labels_filtered
    y_train['kmeans_cluster'] = kmeans_labels_filtered
    ...
    # Ajouter les résultats K-Means aux données
    kmeans_labels = kmeans_results[best_k - 2][2]  # Obtenir les labels du meilleur k
    X_train['kmeans_cluster'] = kmeans_labels
    y_train['kmeans_cluster'] = kmeans_labels

    # 7. Modèle paramétrique global
    X_train_full = pd.concat([X_train.drop(['cluster', 'kmeans_cluster'], axis=1), y_train[['start', 'stop', 'status']]], axis=1)
    aft = WeibullAFTFitter()
    aft.fit(X_train_full, duration_col='stop', event_col='status', entry_col='start', weights_col="poids_longueur")
    aft.print_summary()

    joblib.dump(aft, 'modele_survie_weibull_dbscan_kmeans.pkl')

    print("\n✅ Script terminé avec survie par cohortes, DBSCAN et K-Means et modèle paramétrique global.")
