...
def prediction(CANA_PATH, DEFA_PATH):
    ...
    variables = ['DIAMETRE', 'poids_longueur', 'ANNEEPOSE_YEAR'] + list(materiaux_cols)
    ...
    for cluster_id in sorted(X_train['cluster'].unique()):
        ...
        plt.xlabel("Age (années depuis pose)")
    ...
    aft.fit(X_train_full, duration_col='stop', event_col='status', entry_col='start')
    aft.print_summary()
